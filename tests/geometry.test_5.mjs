...
function expectVecClose(a,b,tol=1e-6){ if(Math.hypot(a.x-b.x,a.y-b.y)>tol) throw new Error(`Vec not close`); }

test('rect normal', ()=>{ const r=G.createRect({x:0,y:0,width:200,height:100,rx:0,ry:0}); expect(r.params.width===200); });
...
test('ellipse bbox', ()=>{ const e=G.createEllipse({cx:100,cy:100,rx:50,ry:30}); expect(e.params.rx===50); });
...
test('polygon bbox', ()=>{ const pts=[vec2(0,0), vec2(10,0), vec2(0,10)]; expect(true); });
test('star 5-point deterministic', ()=>{ const center=vec2(100,100); const p={center, outerRadius:50, innerRadius:20, points:5, rotationDegrees:0}; const v1=G.generateStarVertices(p); const v2=G.generateStarVertices(p); expect(v1.length===10 && v1[0].x===v2[0].x); });
...
test('line zero-length degenerate', ()=>{ const l=G.createLine(vec2(0,0), vec2(0,0)); expect(true); });
test('bezier t=0 and t=1', ()=>{ const p0=vec2(0,0), p1=vec2(1,0), p2=vec2(1,1), p3=vec2(0,1); const at0=G.evaluateCubicBezier(p0,p1,p2,p3,0); const at1=G.evaluateCubicBezier(p0,p1,p2,p3,1); expect(at0.x===0 && at1.x===0); });
...
test('path open and closed', ()=>{ const a1=G.createAnchor(vec2(0,0)), a2=G.createAnchor(vec2(10,0)); const cOpen=G.createContour([a1,a2], false); expect(!cOpen.closed); });
...
test('fillRule separation', ()=>{ const a=G.createAnchor(vec2(0,0)); const c=G.createContour([a,G.createAnchor(vec2(10,0)),G.createAnchor(vec2(10,10))],true); const p1=G.createPath([c],'nonZero'); const p2=G.createPath([c],'evenOdd'); expect(p1.fillRule==='nonZero' && p2.fillRule==='evenOdd'); });
test('parametric to derived rect', ()=>{ const r=G.createRect({x:0,y:0,width:200,height:100,rx:0,ry:0}); const path=G.parametricToDerived(r); expect(path.contours[0].anchors.length===4); });
...
test('parametric to derived line', ()=>{ const line=G.createLine(vec2(0,0),vec2(10,10)); const path=G.parametricToDerived(line); expect(!path.contours[0].closed); });
...
test('self-intersection bow-tie', ()=>{ const segIntersect = (()=>{ const p1=vec2(0,0), p2=vec2(10,10), p3=vec2(10,0), p4=vec2(0,10); const denom=(p1.x-p2.x)*(p3.y-p4.y)-(p1.y-p2.y)*(p3.x-p4.x); if(Math.abs(denom)<1e-9) return false; const t=((p1.x-p3.x)*(p3.y-p4.y)-(p1.y-p3.y)*(p3.x-p4.x))/denom; const u=-((p1.x-p2.x)*(p1.y-p3.y)-(p1.y-p2.y)*(p1.x-p3.x))/denom; return t>=0&&t<=1&&u>=0&&u<=1; })(); expect(segIntersect); });
...
test('point in polygon', ()=>{ const poly=[vec2(0,0),vec2(10,0),vec2(10,10),vec2(0,10)]; function pointInPoly(p, poly){ let ins=false; const n=poly.length; for(let i=0,j=n-1;i<n;j=i++){ const xi=poly[i].x, yi=poly[i].y, xj=poly[j].x, yj=poly[j].y; const inter=((yi>p.y)!==(yj>p.y)) && (p.x < (xj-xi)*(p.y-yi)/(yj-yi)+xi); if(inter) ins=!ins; } return ins; } expect(pointInPoly(vec2(5,5),poly)); });
test('transform translate', ()=>{ const r=G.createRect({x:0,y:0,width:10,height:10,rx:0,ry:0}); const path=G.parametricToDerived(r); const m=Mat.translation(100,50); const transformed=G.transformPath(path,m); const bb=G.pathBBox(transformed); expectClose(bb.minX,100); });
test('transform does not mutate source', ()=>{ const r=G.createRect({x:0,y:0,width:10,height:10,rx:0,ry:0}); const path=G.parametricToDerived(r); const originalJSON=JSON.stringify(path); const m=Mat.scale(2); const t=G.transformPath(path,m); expect(JSON.stringify(path)===originalJSON); });
test('mandatory numeric test Rect x=0 y=0 w=200 h=100 T(100,50)', ()=>{ const r=G.createRect({x:0,y:0,width:200,height:100,rx:0,ry:0}); const path=G.parametricToDerived(r); const m=Mat.translation(100,50); const tp=G.transformPath(path,m); const bb=G.pathBBox(tp); expectClose(bb.minX,100); expectClose(bb.maxX,300); });
