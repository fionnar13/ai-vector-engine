...
import { parseDSL, validateDSL, compileDSL, serializeDSL, DSLExecutor, DSLReferenceEnvironment, lintDSL, validateSchema, DSLErrorCodes } from '../src-js/dsl.js';
...
function test(name, fn){
  ...
  try{
    ...
    if(result && typeof result.then==='function'){
      // async test - handle via await in top-level
...
test('valid reference', ()=>{
  ...
  expect(parse.success, JSON.stringify(parse.errors));
  const validation=validateDSL(parse.program);
  expect(validation.valid, JSON.stringify(validation.errors));
});
...
test('create rect must produce T01', ()=>{
  ...
  expect(compile.success, JSON.stringify(compile.errors));
...
await asyncTest('propose_constraint Proposal returned ConstraintStore unchanged', async ()=>{
  const doc=createDoc();
  const registry=createCoreToolRegistry();
  const createRect=()=>{
    const gid=uuid(); doc.geometryStore.create(gid, {type:'rect', params:{x:0,y:0,width:100,height:100,rx:0,ry:0}});
    ...
    const root=doc.sceneGraph.getAllNodes().filter(n=> !n.parent && !n.parentId)[0] || doc.sceneGraph.createRoot();
...
await asyncTest('rollback on failure atomicity', async ()=>{
  const doc=createDoc();
  const registry=createCoreToolRegistry();
  const docContext={objectStore: doc.objectStore, geometryStore: doc.geometryStore, appearanceStore: doc.appearanceStore, sceneGraph: doc.sceneGraph, semanticStore: doc.semanticStore};
  ...
  const input={version:'1.0', program:[
    ...
    {op:'create', type:'rect', id:'B', args:{width:100, height:100}},
    {op:'boolean', operation:'union', targets:['A','B'], args:{fillRule:'nonZero'}},
    {op:'transform', target:'C', args:{translate:{x:10, y:0}}}
  ]};
  const parse=parseDSL(input);
  expect(parse.success);
  const validation=validateDSL(parse.program);
  ...
  const ir=[
    ...
    {toolId:'T05', input:{objectIds:[], delta:{x:10,y:0}}, sourceInstructionIndex:2, targets:['C'], category:'mutation'}
  ];
...
await asyncTest('vertical slice create red 200x100 rx12 and center', async ()=>{
  const doc=createDoc();
  const registry=createCoreToolRegistry();
  const docContext={objectStore: doc.objectStore, geometryStore: doc.geometryStore, appearanceStore: doc.appearanceStore, sceneGraph: doc.sceneGraph, semanticStore: doc.semanticStore};
