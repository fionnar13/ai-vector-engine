...
export class WorkingCopy {
  constructor(journal){ this.journal=journal; this.objects=new Map(); this.geometries=new Map(); this.appearances=new Map(); this.nodes=new Map(); this.semantics=new Map(); this.constraints=new Map();
    this.originalObjects=new Set(); this.originalGeometries=new Set(); this.originalAppearances=new Set(); this.originalNodes=new Set(); this.originalSemantics=new Set(); this.originalConstraints=new Set();
    this.deletedObjects=new Set(); this.deletedGeometries=new Set(); this.deletedAppearances=new Set(); this.deletedNodes=new Set(); this.deletedSemantics=new Set(); this.deletedConstraints=new Set();
  }
  ...
  loadSemantic(id, canonical){ if(canonical){ this.semantics.set(id, deepClone(canonical)); this.originalSemantics.add(id);} }
  loadConstraint(id, canonical){ if(canonical){ this.constraints.set(id, deepClone(canonical)); this.originalConstraints.add(id);} }
  ...
  getSemantic(id){ const s=this.semantics.get(id); return s?deepClone(s):undefined; }
  getConstraint(id){ const c=this.constraints.get(id); return c?deepClone(c):undefined; }
  ...
  hasSemantic(id){ return this.semantics.has(id); }
  hasConstraint(id){ return this.constraints.has(id); }
  setObject(obj){
  ...
  setSemantic(data){
    if(!this.originalSemantics.has(data.objectId) && !this.semantics.has(data.objectId)) this.journal.add('semantic', data.objectId);
    else if(this.originalSemantics.has(data.objectId) && !this.journal.hasModified('semantic', data.objectId) && !this.journal.hasAdded('semantic', data.objectId)) this.journal.modify('semantic', data.objectId);
    this.semantics.set(data.objectId, deepClone(data));
  }
  setConstraint(constraint){
    const sid=constraint.id || constraint.objectId || constraint;
    const idStr = typeof sid === 'string' ? sid : constraint.id;
    if(!this.originalConstraints.has(idStr) && !this.constraints.has(idStr)) this.journal.add('constraint', idStr);
    else if(this.originalConstraints.has(idStr) && !this.journal.hasModified('constraint', idStr) && !this.journal.hasAdded('constraint', idStr)) this.journal.modify('constraint', idStr);
    this.constraints.set(idStr, deepClone(constraint));
  }
  setNode(node){
  ...
  deleteSemantic(id){
    if(this.semantics.has(id) || this.originalSemantics.has(id)){
      if(this.journal.hasAdded('semantic', id)) this.journal.removeAdded('semantic', id);
      else this.journal.remove('semantic', id);
      this.semantics.delete(id); this.deletedSemantics.add(id);
    }
  }
  deleteConstraint(id){
    const sid=id;
    if(this.constraints.has(sid) || this.originalConstraints.has(sid)){
      if(this.journal.hasAdded('constraint', sid)) this.journal.removeAdded('constraint', sid);
      else this.journal.remove('constraint', sid);
      this.constraints.delete(sid); this.deletedConstraints.add(sid);
    }
  }
  deleteNode(id){
