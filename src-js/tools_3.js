  ...
  execute(input, context){
    ...
    if(context.workingCopy){
      ...
      if(context.sceneGraph){
        try{
          let root=null;
          if(context.sceneGraph.getRoots) { const roots=context.sceneGraph.getRoots(); root=roots[0]; }
          else if(context.sceneGraph.getAllNodes) { const all=context.sceneGraph.getAllNodes(); root=all.find(n=> !n.parentId) || all[0]; }
          if(!root && context.sceneGraph.createRoot) root=context.sceneGraph.createRoot();
          if(root) context.sceneGraph.createNode(oid, root.id);
        }catch{}
      }
      return {success:true, output:{objectId:oid, geometryId:gid, nodeId:nid}};
    }
  ...
  execute(input, context){
    ...
    if(context.sceneGraph && context.sceneGraph.createGroup){
      try {
        ...
        const gid=uuid();
        // Create group manually if createGroup failed
        try {
          if(context.sceneGraph.createNode){
            const rootNodes=context.sceneGraph.getAllNodes ? context.sceneGraph.getAllNodes().filter(n=> !n.parent && !n.parentId) : [];
            const root=rootNodes[0];
            const parentId=root ? root.id : null;
            context.sceneGraph.createNode(null, parentId);
          }
        } catch{}
        return {success:true, output:{groupNodeId:gid}};
      }
