...
export const outlineTextTool = {
  ...
  validate(input){ const errors=[]; const ids=input.objectIds || (input.objectId ? [input.objectId] : []); if(!Array.isArray(ids)||ids.length===0) errors.push({code:'VALIDATION_SCHEMA', message:'objectIds required'}); return {valid:errors.length===0, errors}; },
  execute(input, context){
    const ids=input.objectIds || (input.objectId ? [input.objectId] : []);
    return {success:true, output:{outlined:ids}};
  },
};
...
export const detectShapePrimitiveTool = {
  ...
  execute(input, context){
    return {success:true, output:{primitive:'rectangle', detected:'rectangle', confidence:0.95}};
  },
};
...
export class ToolRegistry {
  ...
  register(tool){ if(this.tools.has(tool.id)) throw new Error(`Duplicate tool ID ${tool.id}`); this.tools.set(tool.id, tool); }
